...
class Arm_Device(object):
    ...
    def idle(self, time):
        try:
            self.Arm_serial_servo_write6(90, 90, 90, 90, 90, 90, time)
        except:
            print('Arm_serial_servo_write6 I2C error') 

    def ready(self, time):
        try:
            self.Arm_serial_servo_write6(90, 130, 0, 0, 270, 60, time)
        except:
            print('Arm_serial_servo_write6 I2C error') 
            
    def target(self, time):
        try:
            self.Arm_serial_servo_write6(90, 53, 28, 40, 270, 60, time)
        except:
            print('Arm_serial_servo_write6 I2C error') 

    def destiny(self, time):
        try:
            self.Arm_serial_servo_write6(136, 57, 30, 20, 270, 145, time)
        except:
            print('Arm_serial_servo_write6 I2C error')          

    def grab(self, time):
        try:
            id1 = self.Arm_serial_servo_read(1)
            id2 = self.Arm_serial_servo_read(2)
            id3 = self.Arm_serial_servo_read(3)
            id4 = self.Arm_serial_servo_read(4)
            id5 = self.Arm_serial_servo_read(5)
            self.Arm_serial_servo_write6(id1, id2, id3, id4, id5, 145, time)
    ...
    def release(self, time):
        try:
            ...
            id4 = self.Arm_serial_servo_read(4)
            id5 = self.Arm_serial_servo_read(5)
            self.Arm_serial_servo_write6(id1, id2, id3, id4, id5, 0, time)
    ...
    def up(self, time):
        try:
            ...
            id4 = id4 + 10
            id5 = self.Arm_serial_servo_read(5)
            id6 = self.Arm_serial_servo_read(6)
            self.Arm_serial_servo_write6(id1, id2, id3, id4, id5, id6, time)

        except:
            print('Arm_serial_servo_write6 I2C error') 
            
            
    def greenBox(self, clock):
        try:
            self.idle(clock)
    ...
    def distGrip(self, dist, time):
        try:
            # Entre 4 y +6cm
            if dist >= 4 and dist < 10:
                #angle = self.Arm_serial_servo_read(6)
                #value = int((3100 - 900) * (angle - 0) / (180 - 0) + 900)
                pos = int((6 * 1511)/(dist))
                # pos = ((pos << 8) & 0xff00) | ((pos >> 8) & 0xff)
                value_H = (pos >> 8) & 0xFF
                value_L = pos & 0xFF
                time_H = (time >> 8) & 0xFF
                time_L = time & 0xFF
                self.bus.write_i2c_block_data(self.addr, 0x10 + 6, [value_H, value_L, time_H, time_L])
                
            # Entre 3 y 4cm
            elif dist < 4 and dist >=3:
                angle = (3*(sqrt(5)*sqrt(dist*(3645*dist + 186736)) - 135*dist))/(11*dist)
                pos = int((3100 - 900) * (angle - 0) / (180 - 0) + 900)
            ...
            # Entre 2 y 3cm
                ...
                angle = (45*(-9*dist + sqrt(dist*(3872 + 81*dist))))/(11*dist)
                pos = int((3100 - 900) * (angle - 0) / (180 - 0) + 900)
            ...
            # Entre 1 y 2cm
                ...
                angle = (45*(-9*dist + sqrt(dist*(2552 + 81*dist))))/(11*dist)
                pos = int((3100 - 900) * (angle - 0) / (180 - 0) + 900)
